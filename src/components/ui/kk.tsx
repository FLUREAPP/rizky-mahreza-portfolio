...
// ShaderBackground — adapted from the provided 21st.dev Shader Builder
// ("Mesh drift"). Zero runtime dependencies: one WebGL canvas fills its parent.
// The palette is tuned for the executive HSE portfolio: deep navy, petroleum
// blue and restrained cyan highlights.
...
vec3 oklabToLin(vec3 c) {
  ...
  float m = c.x - 0.1055613458 * c.y - 0.0638541722 * c.z;
...
const UNIFORMS = {
  // Deep navy / petroleum blue / restrained cyan / soft white.
  colors: [
    [0.027, 0.078, 0.129],
    [0.035, 0.180, 0.294],
    [0.070, 0.380, 0.545],
...
export function ShaderBackground({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const cursorEnabled = UNIFORMS.cursorEnabled && !reducedMotion
      ...
      const resizeCanvas = () => {
        ...
        const pixelScale = Math.min(1, Math.sqrt(1_600_000 / Math.max(1, rawWidth * rawHeight)))
      ...
      function render(now: number) {
        ...
        gl.uniform4f(uni.cursor, cursorEnabled ? cursorPresence : 0, UNIFORMS.cursorEffect, UNIFORMS.cursorStrength, UNIFORMS.cursorRadius)
        gl.drawArrays(gl.TRIANGLES, 0, 3)
        const pointerSettling = Math.abs(targetX - mouseX) > 0.001 || Math.abs(targetY - mouseY) > 0.001 || Math.abs(targetPresence - cursorPresence) > 0.001
